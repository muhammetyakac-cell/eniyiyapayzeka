...
import { runNemotronAgent } from "../src/lib/nemotron-agent";
import * as dotenv from "dotenv";

dotenv.config();
const prisma = new PrismaClient();

const CONCURRENT_NEMOTRON_BOTS = 3; 
...
async function runNemotronBotInstance(botId: number) {
  let loopCount = 1;
  while (true) {
    console.log(`\n🟢 [NEMOTRON Bot #${botId} - Tur #${loopCount}] Uyanıyor ve Nemotron 550B API'ye bağlanıyor...`);
    try {
      const existingTools = await prisma.aiTool.findMany({ select: { name: true } });
      const toolNames = existingTools.map(t => t.name);

      const result = await runNemotronAgent(toolNames);
      ...
      console.log(`✅ [NEMOTRON Bot #${botId}] YENİ ARAÇ: ${createdTool.name}`);
      ...
      const createdBlog = await prisma.blogPost.upsert({
        ...
        create: {
          ...
          readTime: 6,
          author: `Otonom AI (Nemotron Bot ${botId})`,
      ...
      console.log(`✅ [NEMOTRON Bot #${botId}] YENİ BLOG: ${createdBlog.title}`);

    } catch (error: any) {
      console.log(`❌ [NEMOTRON Bot #${botId}] Hata: ${error.message} (Sonraki turda denenecek)`);
    }
...
async function startNemotronSwarm() {
  console.log("==========================================================");
  console.log(`🟢 ${CONCURRENT_NEMOTRON_BOTS} ADET PARALEL NEMOTRON (550B) BOTU BAŞLATILIYOR 🟢`);
  console.log("==========================================================");
  
  for (let i = 1; i <= CONCURRENT_NEMOTRON_BOTS; i++) {
    const startDelay = (i - 1) * 20000;
    setTimeout(() => {
      runNemotronBotInstance(i);
    }, startDelay);
  }
}
