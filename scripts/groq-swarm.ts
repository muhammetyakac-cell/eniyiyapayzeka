...
import { runGroqAgent } from "../src/lib/groq-agent";
...
async function runGroqBotInstance(botId: number) {
  let loopCount = 1;
  while (true) {
    console.log(`\n⚡ [GROQ Bot #${botId} - Tur #${loopCount}] Uyanıyor ve Llama 3.3 (70B) API'ye bağlanıyor...`);
    try {
      const existingTools = await prisma.aiTool.findMany({ select: { name: true } });
      const toolNames = existingTools.map(t => t.name);

      const result = await runGroqAgent(toolNames);
      ...
      console.log(`✅ [GROQ Bot #${botId}] YENİ ARAÇ: ${createdTool.name}`);
      ...
      const createdBlog = await prisma.blogPost.upsert({
        ...
        create: {
          ...
          readTime: 6,
          author: `Otonom AI (Groq Bot ${botId})`,
      ...
      console.log(`✅ [GROQ Bot #${botId}] YENİ BLOG: ${createdBlog.title}`);

    } catch (error: any) {
      console.log(`❌ [GROQ Bot #${botId}] Hata: ${error.message} (Sonraki turda denenecek)`);
    }
...
async function startGroqSwarm() {
  console.log("==========================================================");
  console.log(`⚡ ${CONCURRENT_GROQ_BOTS} ADET PARALEL GROQ (LLAMA-3.3-70B) BOTU BAŞLATILIYOR ⚡`);
  console.log("==========================================================");
  
  for (let i = 1; i <= CONCURRENT_GROQ_BOTS; i++) {
    const startDelay = (i - 1) * 15000;
    setTimeout(() => {
      runGroqBotInstance(i);
    }, startDelay);
  }
}
